...
const Post = ({ post }) => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      const response = await axios.get(
        `${URL}/api/v1/users?userId=${post.userId}`
      );
  ...
  useEffect(() => {
    const fetchLikes = async () => {
      try {
        const response = await axios.get(
          `${URL}/api/v1/posts/${post._id}/likes`
        );
    ...
    const fetchDislikes = async () => {
      try {
        const response = await axios.get(
          `${URL}/api/v1/posts/${post._id}/dislikes`
        );
  ...
  function likeHandler() {
    try {
      axios
        .put(`${URL}/api/v1/posts/${post._id}/like`, {
  ...
  const dislikeHandler = async () => {
    try {
      axios
        .put(`${URL}/api/v1/posts/${post._id}/dislike`, {
          userId: currentUser._id,
        })
        .then((response) => {
